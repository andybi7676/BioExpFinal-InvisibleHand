...
class SignalBthConn():
    ...
    def __init__(self, addr, testThreshold = None):
        self.addr = addr
        super().__init__()
        self.connectToRobot()
        self.dataBuffer = []
    ...
    def disconnectedFromBluetooth(self):
        ...
        if self.testThreshold:
            self.testThreshold.stopCollecting()

    def receivedBluetoothMessage(self):
        while self.sock.canReadLine():
            signal = str(self.sock.readLine())[2:-3].strip()
            if (self.testThreshold and not self.testThreshold.pauseHandling):
                self.testThreshold.handleNewSignal(signal)
...
class TestThreshold():
    ...
    def handleNewSignal(self, newSignal):
        frame = TestThreshold.parseData(newSignal)
        if self.overThreshold(frame) and not self.state:
            print(">>>>Over  Threshold!!>>>>")
            self.state = True
        if not self.overThreshold(frame) and self.state:
...
if __name__ == "__main__":
    ...
    test = TestThreshold()
    sigBth = SignalBthConn("98:D3:81:FD:46:F2", testThreshold=test)